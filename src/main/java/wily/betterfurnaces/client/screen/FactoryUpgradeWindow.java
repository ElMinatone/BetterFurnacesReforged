...
public class FactoryUpgradeWindow extends FactoryScreenWindow<SmeltingScreen<?>> {
    ...
    public List<? extends Renderable> getNestedRenderables() {
        ...
        if (parent.storedFactoryUpgradeType(4)) list.addAll(List.of(
                new FactoryDrawableButton(getX() + 6,getY() + 66, BetterFurnacesDrawables.BUTTON).icon(BetterFurnacesDrawables.getButtonIcon(3)).select(parent.getMenu().be.getRedstoneSetting() == 0).tooltip(Component.translatable("tooltip." + BetterFurnacesReforged.MOD_ID + ".gui_redstone_ignored")).onPress((b,i)-> CommonNetwork.sendToServer(new SettingsSyncPayload(parent.getMenu().getPos(), FactoryUpgradeSettings.Type.REDSTONE, 0, 0))),
                new FactoryDrawableButton(getX() + 22,getY() + 66,BetterFurnacesDrawables.BUTTON).icon(BetterFurnacesDrawables.getButtonIcon(parent.getMenu().be.getRedstoneSetting() == (isShiftKeyDown() ? 1 : 2) ? 5 : 4)).select((parent.getMenu().be.getRedstoneSetting() == 1 || parent.getMenu().be.getRedstoneSetting() == 2) && !isShiftKeyDown()).tooltip(Component.translatable("tooltip." + BetterFurnacesReforged.MOD_ID + ".gui_redstone_" + (isShiftKeyDown() && parent.getMenu().getRedstoneMode() == 1 ? "low" : "high"))).onPress((b,i)-> CommonNetwork.sendToServer(new SettingsSyncPayload(parent.getMenu().getPos(), FactoryUpgradeSettings.Type.REDSTONE, 0, isShiftKeyDown() && parent.getMenu().getRedstoneMode() == 1 ? 2 : 1))),
                new FactoryDrawableButton(getX() + 38,getY() + 66,BetterFurnacesDrawables.BUTTON).icon(BetterFurnacesDrawables.getButtonIcon(7)).select(parent.getMenu().be.getRedstoneSetting() == 3).tooltip(Component.translatable("tooltip." + BetterFurnacesReforged.MOD_ID + ".gui_redstone_comparator")).onPress((b,i)-> CommonNetwork.sendToServer(new SettingsSyncPayload(parent.getMenu().getPos(), FactoryUpgradeSettings.Type.REDSTONE, 0, 3))),
                new FactoryDrawableButton(getX() + 6,getY() + 82,BetterFurnacesDrawables.BUTTON).icon(BetterFurnacesDrawables.getButtonIcon(6)).select(parent.getMenu().be.getRedstoneSetting() == 4).tooltip(Component.translatable("tooltip." + BetterFurnacesReforged.MOD_ID + ".gui_redstone_comparator_sub")).onPress((b,i)-> CommonNetwork.sendToServer(new SettingsSyncPayload(parent.getMenu().getPos(), FactoryUpgradeSettings.Type.REDSTONE, 0, 4)))));
        if (parent.getMenu().getRedstoneMode() == 4) list.add( new FactoryDrawableButton(getX() + 22,getY() + 82,BetterFurnacesDrawables.SURFACE_BUTTON).color(disableSubButton ? Color.WHITE : null).icon(BetterFurnacesDrawables.getButtonIcon( (isShiftKeyDown() ? 10 : 8) + (disableSubButton ? 1 : 0))).tooltip(Component.translatable("tooltip.betterfurnacesreforged.gui_"+ (isShiftKeyDown() ?"sub" : "add") +"_signal") ).onPress((b,i)-> {if (!disableSubButton) CommonNetwork.sendToServer(new SettingsSyncPayload(parent.getMenu().getPos(), FactoryUpgradeSettings.Type.REDSTONE, 1, parent.getMenu().getComSub() + (isShiftKeyDown() ? -1 : 1)));}));
    ...
    private IFactoryDrawableType getSideButtonDrawable(int index){
        int setting = parent.getMenu().be.furnaceSettings.get(FactoryUpgradeSettings.Type.SIDES, index);
    ...
    private FactoryDrawableButton getSideButton(int setting, int index, BlockSide side){
        return new FactoryDrawableButton(getX() + 8 + 14 * (sidePositionIndex(side) % 3),getY() + 22 + 14 * ((int)Math.ceil((double)(sidePositionIndex(side) + 1 )/ 3) - 1),getSideButtonDrawable(index)).tooltips(isShiftKeyDown() ? List.of(Component.translatable("tooltip." + BetterFurnacesReforged.MOD_ID + ".gui_reset")) : List.of(side.getComponent(),getTooltip(setting))).onPress((b, i)-> {
            if (isShiftKeyDown()) CommonNetwork.sendToServer(new SettingsSyncPayload(parent.getMenu().getPos(), FactoryUpgradeSettings.Type.SIDES, IntStream.rangeClosed(0,5).toArray(),0));
            else CommonNetwork.sendToServer(new SettingsSyncPayload(parent.getMenu().getPos(), FactoryUpgradeSettings.Type.SIDES, index, i == GLFW.GLFW_MOUSE_BUTTON_2 ? (setting <= 0 ? 4 : setting - 1) : (setting >= 4 ? 0 : setting + 1)));;
        });
    }
